  ...
  */
  ...
  $.extend(flowplayer, {
    ...
    defaults: {
      ...
      volume: typeof localStorage != "object"
        ? 1
  ...
  $.fn.flowplayer = function (opts, callback) {
    ...
    return !opts && this.data("flowplayer") || this.each(function () {
      ...
      var root = $(this).addClass("is-loading"),
        ...
        videoTag = $("video", root).addClass("fp-engine").removeAttr(
          "controls",
        ),
      ...
      if (conf.playlist.length) { // Create initial video tag if called without
        ...
        if (typeof conf.playlist[0] === "string") {
          videoTag.attr("src", conf.playlist[0]);
        } else {
      ...
      var isRTL =
        (this.currentStyle && this.currentStyle["direction"] === "rtl") ||
        (window.getComputedStyle &&
          window.getComputedStyle(this, null).getPropertyValue("direction") ===
            "rtl");
      ...
      var api = oldApi || {
        ...
        conf: conf,
        currentSpeed: 1,
        volumeLevel: typeof conf.volume === "undefined"
          ? storage.volume * 1
          : conf.volume,
        video: {},
        ...
        seek: function (time, callback) {
          if (api.ready) {
            ...
            time = lastSeekPosition = Math.min(
              Math.max(time, 0),
              api.video.duration,
            ).toFixed(1);
        ...
        seekTo: function (position, fn) {
          var time = position === undefined
            ? lastSeekPosition
            : api.video.duration * 0.1 * position;
        ...
        mute: function (flag) {
          ...
          storage.volume = !isNaN(storage.volume)
            ? storage.volume
            : conf.volume; // make sure storage has volume
        ...
        speed: function (val, callback) {
          if (api.ready) {
            ...
            if (typeof val == "boolean") {
              val = conf
                .speeds[
                  $.inArray(api.currentSpeed, conf.speeds) + (val ? 1 : -1)
                ] || api.currentSpeed;
            }
      ...
      if (!root.data("flowplayer")) { // Only bind once
        root.bind("boot", function () {
          ...
          // splash
          if (
            conf.splash || root.hasClass("is-splash") ||
          ...
          // unload others
          if (conf.splash) {
            $(".flowplayer").filter(".is-ready, .is-loading").not(root).each(
              function () {
          ...
          root.toggleClass("is-splash", !is_ready).toggleClass(
            "is-ready",
            is_ready,
          );
          api.ready = is_ready;
          ...
          function (e, _api, video) {
            ...
            if (e.type == "ready") {
              ...
              if (video) {
                api.paused = !video.duration ||
                  !conf.autoplay && (conf.preload != "none");
              }
            }
            ...
            // CSS classes
            root.toggleClass("is-paused", api.paused).toggleClass(
              "is-playing",
              api.playing,
            );
  ...
  !function () {
    ...
    var s = flowplayer.support,
      ...
      ANDROID_VER = IS_ANDROID
        ? parseFloat(/Android\ (\d\.\d)/.exec(UA)[1], 10)
        : 0;
    $.extend(s, {
      subtitles: !!video.addTextTrack,
      fullscreen: !IS_ANDROID &&
        (typeof document.webkitCancelFullScreen == "function" &&
            !/Mac OS X 10_5.+Version\/5\.0\.\d Safari/.test(UA) ||
      ...
      firstframe: !IS_IPHONE && !IS_IPAD && !IS_ANDROID && !IS_SILK &&
        !IS_IPAD_CHROME && !IS_WP && !IS_ANDROID_FIREFOX,
      inlineVideo: !IS_IPHONE && !IS_SILK && !IS_WP &&
        (!IS_ANDROID || ANDROID_VER >= 3),
    ...
    var engine = {
      ...
      load: function (video) {
        function escapeURL(url) {
          return url.replace(/&amp;/g, "%26").replace(/&/g, "%26").replace(
            /=/g,
            "%3D",
          );
        }
        ...
        if (html5Tag.length > 0 && flowplayer.support.video) {
          html5Tag[0].pause();
        }
        html5Tag.remove();

        // convert to absolute
        if (!is_absolute && !conf.rtmp) {
          url = $("<img/>").attr("src", url)[0].src;
        }
          ...
          setTimeout(function () {
            try {
              if (!api.PercentLoaded()) {
                return root.trigger("error", [player, {
                  code: 7,
                  url: conf.swf,
                }]);
              }
    ...
    player.bind("ready fullscreen fullscreen-exit", function (e) {
      ...
      if (player.conf.flashfit || /full/.test(e.type)) {
        ...
        if (player.conf.flashfit || e.type === "fullscreen") {
          ...
          vmargin = Math.max(
            Math.round((screenH + vmargin - objheight) / 2),
            0,
          );
  ...
  flowplayer.engine.html5 = function (player, root) {
    ...
    return self = {
      pick: function (sources) {
        if (support.video) {
          if (conf.videoTypePreference) {
            var mp4source = findFromSourcesByType(
              sources,
              conf.videoTypePreference,
            );
      ...
      load: function (video) {
        if (conf.splash && !api) {
          ...
          if (player.video.src && video.src != player.video.src) {
            ...
            if (support.zeropreload) {
              player.trigger("ready", video).trigger("pause").one(
                "ready",
                function () {
                  root.trigger("resume");
                },
        ...
        if (
          conf.preload != "none" || !support.zeropreload || !support.dataload
        ) api.load();
    ...
    function listen(api, sources, video) {
      ...
      if (
        api.listeners && api.listeners.hasOwnProperty(root.data("fp-player_id"))
      ) return;
      ...
      $.each(EVENTS, function (type, flow) {
        api.addEventListener(type, function (e) {
          ...
          if (
            flow == "progress" && e.srcElement && e.srcElement.readyState === 0
          ) {
            setTimeout(function () {
          ...
          if (conf.debug && !/progress/.test(flow)) {
            console.log(type, "->", flow, e);
          }

          // no events if player not ready
          if (
            !player.ready && !/ready|error/.test(flow) || !flow ||
            !$("video", root).length
          ) return;
          ...
          switch (flow) {
            case "ready":
              ...
              if (
                !arg.duration && !support.hlsDuration && type === "loadeddata"
              ) {
  ...
  $.fn.slider2 = function (rtl) {
    ...
    return this.each(function () {
      var root = $(this),
        ...
        fire = function (value) {
          if (
            !disabled && value != api.value && (!maxValue || value < maxValue)
          ) {
        ...
        mousemove = function (e) {
          var pageX = e.pageX;
          if (
            !pageX && e.originalEvent && e.originalEvent.touches &&
        ...
        toDelta = function (value) {
          return Math.max(
            0,
            Math.min(size, vertical ? (1 - value) * height : value * width),
          );
        },
        /* public */
        api = {
  ...
  flowplayer(function (api, root) {
    ...
    function setRatio(val) {
      if (
        (root.css("width") === "0px" || root.css("height") === "0px") ||
        val !== flowplayer.defaults.ratio
      ) {
    ...
    api.bind("ready", function () {
      ...
      ((duration >= 3600) && root.addClass("is-long")) ||
        root.removeClass("is-long");
      ...
      if (error) {
        ...
        $("p", el).text(
          error.url || api.video.url || api.video.src ||
            conf.errorUrls[error.code],
        );
    ...
    var bc = root.css("backgroundColor"),
      has_bg = root.css("backgroundImage") != "none" ||
...
'\
   ...
   ',
    ...
    if (api.conf.tooltip) {
      $(".fp-ui", root).attr("title", "Hit ? for help").on(
        "mouseout.tip",
        function () {
  ...
  var VENDOR = $.browser.mozilla ? "moz" : "webkit",
    ...
    ua = navigator.userAgent.toLowerCase(),
    IS_SAFARI = /(safari)[ \/]([\w.]+)/.exec(ua) &&
      !/(chrome)[ \/]([\w.]+)/.exec(ua);

  // esc button
  $(document).bind(
    FS_NATIVE_SUPPORT ? "fullscreenchange" : VENDOR + "fullscreenchange",
    function (e) {
      var el = $(
      ...
      if (FS_SUPPORT) {
        if (flag) {
          if (FS_NATIVE_SUPPORT) {
            ...
            if (
              IS_SAFARI && !document.webkitCurrentFullScreenElement &&
              !document.mozFullScreenElement
            ) { // Element.ALLOW_KEYBOARD_INPUT not allowed
  ...
  flowplayer(function (player, root) {
    ...
    player.play = function (i) {
      ...
      player.load(
        typeof player.conf.playlist[i] === "string"
          ? player.conf.playlist[i].toString()
          : $.map(player.conf.playlist[i], function (item) {
            return $.extend({}, item);
          }),
      );
      return player;
    ...
    if (els().length) {
      ...
      player.bind("load", function (e, api, video) {
        ...
        // index
        root.removeClass("video" + prevIndex).addClass("video" + index)
          .toggleClass("last-video", is_last);
  ...
  flowplayer(function (player, root, engine) {
    ...
    function seconds(timecode) {
      ...
      return els[0] * 60 * 60 + els[1] * 60 +
        parseFloat(els[2].replace(",", "."));
    }
    ...
    $.get(url, function (txt) {
      for (
        ...
        if (timecode) {
          ...
          // text
          text = "<p>" + lines[++i] + "</p><br/>";
          while ($.trim(lines[++i]) && i < lines.length) {
            text += "<p>" + lines[i] + "</p><br/>";
          }
    ...
    player.bind("cuepoint", function (e, api, cue) {
      ...
      if (
        currentPoint && player.cuepoints[currentPoint] &&
        player.cuepoints[currentPoint].time > time
      ) {
        wrap.removeClass("fp-active");
        currentPoint = null;
      }
      $.each(player.cuepoints || [], function (i, cue) {
        ...
        if (entry && currentPoint != cue.index) {
          ...
          cue.subtitleEnd && time >= cue.time && cue.index == currentPoint + 1
        ) player.trigger("cuepoint", cue);
      });
    });
  });
  ...
  if (flowplayer.support.touch || isIeMobile) {
    flowplayer(function (player, root) {
      var isAndroid = /Android/.test(UA) && !/Firefox/.test(UA) &&
          !/Opera/.test(UA),
        isSilk = /Silk/.test(UA);
      ...
      // native fullscreen
      if (
        player.conf.native_fullscreen && ($.browser.webkit || $.browser.safari)
      ) {
  ...
  flowplayer(function (player, root) {
    ...
    var conf = player.conf,
      ui = $(".fp-ui", root),
      trigger = $("<a/>", { "class": "fp-embed", title: "Copy to your site" })
        .appendTo(ui),
...
flowplayer(function (e, o) {
  ...
  var n = jQuery,
    t = e.conf,
    i = t.swf.indexOf("flowplayer.org") && t.e && o.data("origin"),
    l = i ? a(i) : location.hostname,
    r = t.key;
  if (
    "file:" == location.protocol && (l = "localhost"),
      e.load.ed = 1,
      t.hostname = l,
      t.origin = i || location.href,
      i && o.addClass("is-embedded"),
      "string" == typeof r && (r = r.split(/,\s*/)),
      r && "function" == typeof key_check && key_check(r, l)
  ) {
    t.logo &&
      o.append(
        n("<a>", { "class": "fp-logo", href: i }).append(
          n("<img/>", { src: t.logo }),
        ),
      );
  } else {
    ...
    n(".fp-controls", o),
      e.bind("pause resume finish unload", function (e, o) {
        var a = o.video.src ? o.video.src.indexOf("://my.flowplayer.org") : -1;
        /pause|resume/.test(e.type) && "flash" != o.engine && 4 != a && 5 != a
          ? (s.show().css({
            position: "absolute",
            left: 16,
            bottom: 36,
