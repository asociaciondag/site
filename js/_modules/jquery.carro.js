...
Plugin.prototype = {
  init: function () {
    ...
    if (this.settings.buttons) {
      ...
      if (this.settings.buttonActiveClass) {
        this.$slides.on("enter." + pluginName, function () {
          self.$buttons.filter('[data-carro="' + $(this).index() + '"]')
            .addClass(self.settings.buttonActiveClass);
        });

        this.$slides.on("leave." + pluginName, function () {
          self.$buttons.filter('[data-carro="' + $(this).index() + '"]')
            .removeClass(self.settings.buttonActiveClass);
        });
      }
    }
  ...
  goto: function (position) {
    const $target = this.getSlide(position);
  ...
  getSlide: function (position) {
    ...
    if (
      typeof position === "object" && position.jquery &&
      position.parent().is(this.$tray)
    ) {
      return position;
    }
